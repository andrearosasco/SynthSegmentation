...
if __name__ == '__main__':
    ...
    # {'ycb': '../../Downloads/YCB_Video_Dataset'}
    # valid_set = TestSet(splits=Path('./data'), paths=None, transform=T.Compose([
    #                                           T.Resize(256, InterpolationMode.BILINEAR),
    #                                           T.ToTensor(),
    #                                           T.Normalize(mean=[0.485, 0.456, 0.406],
    #                                                       std=[0.229, 0.224, 0.225])
    #                                                       ]),
    #                      target_transform=T.Compose([T.Resize(256, InterpolationMode.NEAREST),
    #                                                  lambda x: torch.tensor(np.array(x), dtype=torch.long)]))

    valid_set = TrainSet(transform=T.Compose([
        T.Resize(256, InterpolationMode.BILINEAR),
        T.ToTensor(),
        T.Normalize(mean=[0.485, 0.456, 0.406],
                    std=[0.229, 0.224, 0.225])
    ...
    res = valid(valid_loader, model, metrics)
    for m in metrics:
        print(m.compute())
